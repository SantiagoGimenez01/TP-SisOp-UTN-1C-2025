...
void enviar_estado_proc_kernel(uint32_t pid, uint32_t pc, uint32_t* timer_exec) {
    ...
    agregar_int_a_paquete(paquete, timer_exec);
...
void ejecutar_ciclo(uint32_t pid, uint32_t pc, uint32_t* timer_exec)
{
    ...
    while (ejecutando)
    {
        ...
        if (check_interrupt())
        {
            ...
            enviar_estado_proc_kernel(pid, pc, timer_exec);
