...
void inicializar_memoria()
{
    ...
    log_debug(logger, "Memoria inicializada con %d frames de %d bytes", cantidad_frames, configMEMORIA.tam_pagina);
...
void crear_estructuras_para_proceso(uint32_t pid, char *nombre_archivo, int tamanio, uint32_t paginas_necesarias)
{
    ...
    log_debug(logger, "Se inicializaron estructuras, se asignaron marcos y se cargaron instrucciones para el proceso %d", pid);
}
    ...
    for (int i = 0; i < configMEMORIA.entradas_por_tabla; i++)
    {
        ...
        if (nivel_actual == configMEMORIA.cantidad_niveles) {
            ...
            if (pagina->presencia && pagina->marco >= 0 && pagina->marco < cantidad_frames)
            {
                ...
                log_trace(logger, "Bit limpio correctamente en marco %d", pagina->marco);
            }
        ...
        {
            liberar_marcos_de_proceso((t_tabla_nivel *)entrada, nivel_actual + 1);
